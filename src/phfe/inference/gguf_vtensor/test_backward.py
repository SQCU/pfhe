...
def test_training_loop_iq3_xxs():
    """
    Test a complete training loop with quantized weights.

    Uses Q4_0 for stability (IQ3_XXS requant has numerical issues).
    The key point is proving gradient flow works through quantized weights.
    """
    print("\n" + "=" * 60)
    print("TEST: Training Loop with Quantized Weights (Q4_0)")
    print("=" * 60)
    ...
    class QuantizedMLP(nn.Module):
        def __init__(self, dim=256):
            ...
            # Create random weights and quantize to Q4_0 (more stable)
            w1 = torch.randn(dim, dim, device=device) * 0.02
            w2 = torch.randn(dim, dim, device=device) * 0.02

            w1_quant = requant_q4_0_cuda(w1, "q4_0")
            w2_quant = requant_q4_0_cuda(w2, "q4_0")

            self.fc1 = QuantizedLinear(dim, dim, w1_quant, "q4_0", device=device)
            self.fc2 = QuantizedLinear(dim, dim, w2_quant, "q4_0", device=device)
    ...
    # Training loop
    losses = []
    # Large LR needed to overcome Q4_0 quantization floor
    # Small updates get lost to requantization - this is expected!
    # In practice, use CarryOptimizer or larger batches
    lr = 1.0

    print("\nTraining Q4_0 MLP (large LR to overcome quant floor)...")
    ...
    print("Training through quantized weights: ✓")
...
def test_carry_buffer_accumulation():
    ...
    for i in range(100):
        # Gradient that accumulates to exceed flush threshold
        grad = torch.randn(shape, device=device) * 0.1
        qparam.grad_holder.grad = grad
        qparam.apply_gradient(lr=0.1)
