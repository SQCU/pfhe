...
class DequantSTEWithGradCapture(Function):
    ...
    Uses a dummy gradient carrier tensor to enable autograd tracking,
    since the raw quantized bytes (uint8) cannot require grad.
    """

    @staticmethod
    def forward(
        ctx,
        grad_carrier: torch.Tensor,  # Dummy tensor that requires_grad
        raw_data: torch.Tensor,
        shape: Tuple[int, ...],
        quant_type: str,
        numel: int,
    ...
    def backward(ctx, grad_output: torch.Tensor) -> Tuple[torch.Tensor, None, None, None, None, None]:
        ...
        # Return zero gradient for carrier (shape [1])
        return torch.zeros(1, device=grad_output.device), None, None, None, None, None
...
class QuantizedParameter(nn.Module):
    ...
    def forward(self) -> torch.Tensor:
        ...
        """
        # Create grad carrier - a dummy tensor that requires grad to enable autograd
        grad_carrier = torch.zeros(1, device=self.raw_data.device, requires_grad=True)
